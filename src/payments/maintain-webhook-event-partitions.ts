...
/**
 * Creates the `webhook_events` monthly partitions for the current month plus the
 * next three, so provider-callback writes never fall into the catch-all default
 * partition under normal operation. Idempotent (CREATE TABLE IF NOT EXISTS),
 * safe to run on a schedule. `webhook_events` is partitioned by `received_at`.
 */
    ...
    await pool.query(`
      ...
      BEGIN
        FOR offset_month IN 0..3 LOOP
          ...
          partition_name := 'webhook_events_' || to_char(month_start, 'YYYY_MM');
          EXECUTE format(
