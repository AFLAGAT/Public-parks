...
- [x] **Create standard request validation layer.** **Priority:** Critical. **Purpose:** Validate and normalize all external input at the API boundary.
  ...
  - **Notes:** Global `ZodValidationPipe` (`APP_PIPE`) + `ValidationExceptionFilter` (`APP_FILTER`) registered via `ValidationModule` in `src/common/validation/`. `createZodDto(schema)` colocates schema with DTO; `.strict()` schemas REJECT unknown fields (OWASP A03/A04). Canonical `{ error: { code: 'VALIDATION_FAILED', message, details, correlationId } }` envelope at HTTP 400; `correlationId` is placeholder `'not-yet-wired'` until structured-logging item. No new npm dependency introduced.
