...
- [x] **Create standard request validation layer.** **Priority:** Critical. **Purpose:** Validate and normalize all external input at the API boundary.
  ...
  - **Notes:** Global `ZodValidationPipe` (`APP_PIPE`) + `ValidationExceptionFilter` (`APP_FILTER`) registered via `ValidationModule` in `src/common/validation/`. `createZodDto(schema)` colocates schema with DTO; `.strict()` schemas REJECT unknown fields (OWASP A03/A04). Canonical `{ error: { code: 'VALIDATION_FAILED', message, details, correlationId } }` envelope at HTTP 400; `correlationId` is populated by the structured-logging middleware. No new npm dependency introduced by this item.

- [x] **Create standard response and pagination helpers.** **Priority:** High. **Purpose:** Enforce consistent list shapes, page sizes, cursors, and metadata across APIs.
  - **Performance considerations:** Default page sizes must prevent unbounded scans and oversized payloads.
  - **Notes:** Added shared pure helpers under `src/common/pagination/`: strict `cursor`/`pageSize` query schema (default 25, maximum 100); versioned canonical Base64URL cursors capped at 2,048 characters and validated by an endpoint-owned Zod keyset schema; and a fetch-`pageSize + 1` response builder returning `{ data, pagination: { nextCursor, hasMore } }`. Malformed, non-canonical, oversized, unsupported-version, or wrong-endpoint cursors use the existing `RequestValidationException` path with a stable `cursor` detail. Producer misuse (empty/non-scalar/oversized payload or invalid page size) fails immediately. Implemented when the active facility-discovery slice first needed bounded list responses; no dependency added. Verification: 149/149 unit tests, typecheck, lint, and production build pass.
...
- [x] **Set up authentication middleware skeleton.** **Priority:** Critical. **Purpose:** Make authenticated-by-default routing possible from the first endpoint.
...
- [x] **Model staff assignments by facility and time range.** **Priority:** Critical. **Purpose:** Enforce server-side facility scope for staff actions and offline sync.
  ...
  - **Notes:** Added `staff_assignments` (migration `0005_sharp_synch.sql`): a staff `user_id` scoped to a `facility_id` across an explicit `starts_at`/`ends_at` window, with `assignment_status` (`active`/`revoked`), accountability FKs (`assigned_by_user_id`, `revoked_by_user_id`), and all FKs `ON DELETE restrict` to preserve history. A time-range check rejects inverted/empty windows; a revocation-consistency check binds `revoked_at`/`revoked_by_user_id` to the `revoked` status; a partial unique index (`__where_active`) permits at most one active assignment per staff+facility while allowing reassignment after revocation. Built now (ahead of the resident-facing entrance-ticket models, which touch stop-and-ask tables and await owner schema confirmation) because it is the unblocked staff-check-in prerequisite of the active slice and depends only on already-modeled `users`/`facilities`. The server-side scope predicate (active + in-window) is proven by real-Postgres failure-mode tests: revoked, not-yet-started, ended, and cross-facility assignments are all denied; the enforcing repository/service is owned by Phase 4 "Enforce staff facility scope server-side." Verification: 174 unit, 45 integration (8 new), typecheck, lint, and production build pass.
...
- [x] **Model entrance tickets separately.** **Priority:** Critical. **Purpose:** Represent date, facility, quantity, status, buyer, QR issuance, and capacity consumption for pools and parks.
  ...
  - **Notes:** Added `entrance_tickets` (migration `0008_new_bulldozer.sql`) with `facility_id`, `buyer_user_id`, `visit_date`, `quantity`, `used_quantity` (quantity-aware consumption incremented by `check_ins`), the eight-value `entrance_ticket_status` enum (payment state is the ticket status; no separate payment FK — payments reference the ticket polymorphically), and integer-santim price snapshots (`unit_price_at_booking`, `total_amount_at_booking`). Indexed by `(facility_id, visit_date, status)` and `(buyer_user_id, visit_date)`. A composite FK `(facility_id, visit_date) → facility_capacities(facility_id, service_date)` guarantees no ticket without a capacity row; checks enforce `quantity >= 1`, `used_quantity ≤ quantity`, and `total = unit × quantity`. QR issuance is via the existing polymorphic `qr_codes.scannable_type = 'entrance_ticket'`. **Owner-confirmed schema change (stop-and-ask table)** — money representation, full status enum, and composite FK approved 2026-07-05; logged in DECISIONS.md. Facility-name snapshot deferred to the data-snapshot-fields item; lifecycle transitions to the Phase 6 state machine. Verification: 187 unit, 63 integration (7 new), typecheck, lint, and production build pass.

- [x] **Model daily facility capacity.** **Priority:** Critical. **Purpose:** Track available and sold entrance capacity per facility and date.
  ...
  - **Notes:** Added `facility_capacities` (migration `0008_new_bulldozer.sql`): one row per `(facility_id, service_date)` with `max_capacity` and `sold_count`. The `(facility_id, service_date)` uniqueness is a UNIQUE CONSTRAINT (`uq_…`, not a bare index) so it can serve as the `entrance_tickets` composite FK target; a `sold_count between 0 and max_capacity` check is the DB backstop. The confirmed oversell-safe purchase is the atomic conditional update (`... SET sold_count = sold_count + :qty WHERE ... AND sold_count + :qty <= max_capacity RETURNING id`; no row = sold out). **Owner-confirmed schema change (stop-and-ask table).** Real-Postgres failure-mode tests prove the atomic update never oversells and that under a concurrent race for the last unit exactly one purchase wins with `sold_count` ending correct — partially satisfying the separate "Define entrance capacity constraints" item, whose remaining scope is business-logic enforcement in Phase 6. Verification as above.

- [x] **Model payments as polymorphic records.** **Priority:** Critical. **Purpose:** Let one payment system handle slot reservations, shared reservation participant payments, and entrance tickets.
  ...
  - **Notes:** Added `payments` (migration `0009_supreme_shriek.sql`), the aggregate — one per payable (unique `(payable_type, payable_id)`), polymorphic `payable_type`/`payable_id` (no FK, validated in service layer), integer-santim `amount`/`refunded_amount` (bounds-checked), nine-value `payment_status` enum. A **write-once trigger** (`trg_payments__reject_core_mutation`) rejects any UPDATE to `amount`/`payable_type`/`payable_id`/`payer_user_id` — the "immutably enough for reconciliation" requirement enforced at the DB layer — while status/refund/verification stay mutable. **Owner-confirmed stop-and-ask change** (money = integer santim; logged in DECISIONS.md). Real-Postgres failure-mode tests prove core-field mutation is rejected, non-core updates succeed, one-payment-per-payable and refund bounds hold, and the payer FK is enforced. Verification: 193 unit, 68 integration (5 new), typecheck, lint, and production build pass.

- [x] **Model payment attempts separately from payment aggregate state.** **Priority:** Critical. **Purpose:** Preserve retry history, provider references, callbacks, and failure reasons without overwriting important payment evidence.
  - **Dependencies:** Payments, webhook events, reconciliation jobs.
  - **Notes:** Added `payment_attempts` (migration `0009`): one row per charge attempt (FK → `payments`, restrict) preserving `attempt_number`, `provider_key`, per-attempt `amount`, `attempt_status`, `failure_reason`, and provider references. A globally unique `merchant_reference` (our order id) and a partial-unique `provider_transaction_id` (unique only when present) prevent duplicate provider orders/transactions from being recorded twice; `(payment_id, attempt_number)` is unique. Retries are attempts here, never new `payments` rows. Tests prove each uniqueness rule and that unresolved attempts (null transaction id) coexist. Verification as above.

- [x] **Model Telebirr webhook events.** **Priority:** Critical. **Purpose:** Persist every provider callback for idempotency, ordering analysis, reconciliation, and dispute investigation.
  ...
  - **Notes:** Added `webhook_events` (migration `0009`), an append-only log of every callback — provider event id, normalized idempotency key, verification result, processing status, and **redacted** `raw_payload` jsonb — month-partitioned by `received_at` (high-growth per the scaling decision), with current/next/default partitions and `npm run webhooks:maintain-partitions`. **Idempotency-vs-partitioning conflict (owner-confirmed):** because a partitioned table cannot hold a global unique on the non-partition `provider_event_id`, exactly-once is enforced by a separate provider-agnostic, non-partitioned, **immutable** `processed_provider_events` ledger (unique `(provider_key, provider_event_id)`, mutation/truncate triggers) that webhook processing reserves transactionally before acting — a duplicate delivery hits the unique constraint and is skipped. Logged in DECISIONS.md. Real-Postgres tests prove global dedup, ledger immutability (UPDATE/DELETE rejected), and correct monthly-partition routing with the default safety net. Verification as above.

- [x] **Model QR codes as polymorphic access credentials.** **Priority:** Critical. **Purpose:** Issue one QR pattern that can reference either a slot reservation or an entrance ticket.
  ...
  - **Notes:** Added `qr_codes` (migration `0006_slimy_greymalkin.sql`): one credential references either booking type through the approved `scannable_type`/`scannable_id` pair. `scannable_type` is the `qr_scannable_type` enum (`slot_reservation`, `entrance_ticket`) — an enum, not a free varchar — so a QR can never be minted against a non-scannable entity (the confirmed independently-constrained-pair property). Per the confirmed QR token design (DECISIONS: "signed token contains only the QR record ID"), **no reusable secret is stored** — the signed token carries only the row `id`; the `used`-flag intent is realized by the `check_ins` idempotency-keyed scan ledger (next item), which is quantity-aware, rather than a boolean here that could not represent multi-entry entrance tickets. The row carries only `qr_code_status` (`active`/`revoked`) with a revocation-consistency check and `created_at` as issuance time. A partial unique index (`__where_active`) enforces at most one active credential per booking (blocks a duplicate replayable credential) while allowing reissuance after revocation. Polymorphic by design, so there is intentionally no FK; the referenced booking's existence is validated at scan time in Phase 6. Real-Postgres failure-mode tests prove: a non-scannable type is rejected, a revoked credential is excluded from the active-credential lookup, the one-active-per-booking invariant holds (with post-revocation reissuance), revocation-state inconsistency is rejected, and the invariant is scoped per `(type, id)`. Verification: 178 unit, 50 integration (5 new), typecheck, lint, and production build pass.

- [x] **Model check-ins with idempotency keys.** **Priority:** Critical. **Purpose:** Prevent duplicate scans or offline replay from creating multiple attendance records.
  ...
  - **Notes:** Added `check_ins` (migration `0007_thin_purple_man.sql`), the scan/attendance ledger written by online staff validation and offline sync push. Records the required fields: `staff_user_id`, `device_id`, `facility_id` (scope), `validation_source` (`online`/`offline_sync`), `qr_code_id`, `check_in_result` (`accepted`/`rejected`) with a bound `rejection_reason`, and `sync_batch_id` (nullable; FK deferred until `sync_batches` is modeled). The confirmed idempotency key `device_id + qr_code_id + scan_time_rounded_to_minute` is enforced by `uidx_check_ins__idempotency (device_id, qr_code_id, scan_minute)`; `scan_minute` is the minute-truncated scan time held in a plain column with a `date_trunc` CHECK. **Partitioning:** the table is month-partitioned per the confirmed database-scaling decision, but partitioned by `scan_minute` rather than `created_at` — a Postgres unique index on a partitioned table must include the partition key, so partitioning by the idempotency-key's minute is what makes global replay dedup enforceable. PK is `(id, scan_minute)`. The migration creates current/next/default partitions (mirroring `audit_logs`); `npm run checkins:maintain-partitions` (`src/sync/maintain-check-in-partitions.ts`) provisions three months ahead. Exact offline replays are absorbed by the unique index; distinct/conflicting scans are recorded as separate rows (accepted vs. rejected), never dropped, per the confirmed sync-conflict rule. Real-Postgres failure-mode tests prove replay dedup, distinct/rejected coexistence, the `scan_minute` truncation and rejection-reason checks, FK protection, and correct monthly-partition routing with the default safety net. Verification: 181 unit, 56 integration (6 new), typecheck, lint, and production build pass.
...
- [x] **Implement payment provider abstraction with a mock/sandbox adapter.** **Priority:** Critical. **Purpose:** Let every payment-dependent item below (state machine, atomicity, QR issuance, capacity consumption) be built and tested without waiting on Telebirr merchant account approval. **See DECISIONS.md — mock-first, real Telebirr adapter swaps in behind the same interface in Phase 7.**
  ...
  - **Notes:** Added the `PaymentProvider` port (`src/payments/payment-provider.port.ts`, DI token `PAYMENT_PROVIDER`) with provider-agnostic contract types and a real deterministic `MockPaymentProvider` bound to the token in `PaymentsModule`; the Phase 7 Telebirr adapter swaps in at that single point. `initiatePayment` is success-or-`failed` and never `confirmed` (confirmation is always async, modeling Telebirr's `preOrder` → `prepay_id` → `notify_url` flow); `verifyCallback` is a pure, synchronous verify-and-normalize returning `verified`/`signature_invalid`/`unrecognized` (HMAC stand-in for the real RSA-PSS), surfacing the provider-reported amount for the caller to compare and a redacted payload for `webhook_events`. Exactly-once is deliberately NOT here — it stays with the `processed_provider_events` ledger, so the provider is pure and duplicate/out-of-order deliveries never double-confirm. Refund is intentionally deferred (not in the active slice; Telebirr refund mechanics unconfirmed) as a future additive method. **No schema change** (maps to existing `payment_attempts` columns). Critical-path failure-mode unit tests: initiation failure without throw, tampered-amount → signature break, forged signature, unrecognized/malformed payload, duplicate-delivery determinism. Verification: 13 new tests, 206/206 unit, typecheck, lint, and production build pass.
...
- [x] **Define entrance ticket state machine.** **Priority:** Critical. **Purpose:** Make pending payment, confirmed, partially used, fully used, canceled, expired, refunded, and dispute states explicit.
  - **Dependencies:** FacilityCapacity, payments, QR codes, check-ins.
  - **Notes:** Added `EntranceTicketStatePolicy` (`src/entrance-ticketing/entrance-ticket-state.policy.ts`), a pure application-layer guard over the confirmed eight-value `entrance_ticket_status` enum (TS union derived from `entranceTicketStatus.enumValues` — no drift). Transition map resolved and logged in DECISIONS.md ("Entrance ticket state machine (transition map)") per the Decision Velocity Rule: `pending_payment → {confirmed, expired, canceled}`; `confirmed → {partially_used, fully_used, canceled, refunded, disputed}`; `partially_used → {fully_used, refunded, disputed}`; `fully_used → {disputed}`; `canceled`/`expired`/`refunded`/`disputed` terminal. `classifyTransition` returns `legal`/`noop`/`illegal` with **`from === to` always `noop`** (idempotent replay — duplicate payment-verified webhook cannot double-confirm). Money edge (conservative default, revisable at the refund flow): direct `refunded` reachable from `confirmed`/`partially_used` but not `fully_used`. Pure policy — gates status only, no DB writes / no money movement. **No schema change** (enum pre-exists). Critical-path failure-mode unit tests: illegal skips, terminal-state resurrection, `fully_used → refunded` rejection, un-use/cancel-after-use rejection, idempotent self-noop, full enum coverage. Verification: 43 new tests, 249/249 unit, typecheck, lint, and production build pass. Note: NamingConventions "Ticket Status" nine-value list (incl. `partially_refunded`) is stale vs. the eight-value DB enum; defined over the real enum, flagged in DECISIONS.md.
